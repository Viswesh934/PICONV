...
export function pdfToBase64(path: string): string {
  const file = fs.readFileSync(path);
  return file.toString("base64");
}

/** Convert Base64 string back to PDF file */
export function base64ToPdf(base64: string, outputPath: string): void {
  const buffer = Buffer.from(base64, "base64");
  fs.writeFileSync(outputPath, buffer);
}

/** Convert a PDF file to Hex string */
export function pdfToHex(path: string): string {
  const file = fs.readFileSync(path);
  return file.toString("hex");
}

/** Convert Hex string back to PDF file */
export function hexToPdf(hex: string, outputPath: string): void {
  const buffer = Buffer.from(hex, "hex");
  fs.writeFileSync(outputPath, buffer);
}

/** Convert a PDF file to ASCII85 string */
export function pdfToAscii85(path: string): string {
  const file = fs.readFileSync(path);
  return bufferToAscii85(file);
}

/** Convert ASCII85 string back to PDF file */
export function ascii85ToPdf(ascii85: string, outputPath: string): void {
